...
from keras.layers import Lambda,Bidirectional,GRU,Reshape,GlobalAveragePooling1D,concatenate,multiply,Dropout
...
def TARM():
    ...
    if K.image_dim_ordering() == 'tf':
      concat_axis = 3
      img_input = Input(shape=(50, 50, 3), name='data')
    else:
      concat_axis = 1
      img_input = Input(shape=(3, 50, 50), name='data')
    ...
    lstm_output = Bidirectional(
        GRU(nb_lstm, kernel_initializer='orthogonal', recurrent_initializer='orthogonal', dropout=0.5,
    ...
    lstm1 = Reshape((50, 50, 1))(lstm1)
    lstm2 = Reshape((50, 50, 1))(lstm2)
    lstm3 = Reshape((50, 50, 1))(lstm3)
...
class ResnetBuilder(object):
    ...
    def build(input_shape, num_outputs, block_fn, repetitions):
        ...
        TARM_output,img_input=TARM()

        ## Spatial Convolution Module(SCM)
        conv1 = _conv_bn_relu(filters=64, kernel_size=(5, 5), strides=(2, 2))(TARM_output)
...
def MANs_model():
    # input dimensions
    input_rows, input_cols = 50,50
    input_channels = 3
    nb_classes=60
    model = ResnetBuilder.build_resnet_18((input_cols, input_rows, input_channels), nb_classes)
